    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                sys.exit()
            if event.key == pygame.K_d: #движение вправо
                gun.mright = True
...
def create_army(screen, aliens):
    ...
    number_alien_y = int((620 - 100 - 2 * alien_height) / alien_height)
    for alien_row in range(number_alien_y):
        for alien_num in range(number_alien_x):
            alien = Alien(screen)
            alien.x = alien_width + alien_width * alien_num
            alien.y = alien_height + alien_height * (alien_row+1)
            alien.rect.x = alien.x
            alien.rect.y = alien.rect.height + alien.rect.height*(alien_row+1)
